...
function viewprofile()
{
  ...
  {
    $sql="select * from pump_man_reg where man_id='$id'";
    $result = mysql_query($sql);
    $array = mysql_fetch_array($result);
    return $array;
  }
...
if (isset($_POST['editpro']))
{
  ...
  else if ($type=='pump manager') 
  {
    ...
    $phno=$_POST['phno'];
      ...
      $re=mysql_query($s);
      $s="update pump_man_reg set pass='$pass' where man_id='$id'";  
      $re=mysql_query($s);
      $s="update pump_man_reg set repass='$pass' where man_id='$id'";  
      $re=mysql_query($s);
      ...
      $s="update login_pump set name='$name' where id='$id'";  
      $re=mysql_query($s);
      $s="update login_pump set pass='$pass' where id='$id'";  
      $re=mysql_query($s);
      //$_SESSION['name']=$name;
      echo "<script>alert('Details Updated');</script>";
      echo "<script>location.href='manager_home.php';</script>";
  ...
  elseif ($type=='user') 
  {
    ...
    $re=mysql_query($s);
    $s="update user_reg set pass='$pass' where us_id='$id'";  
    $re=mysql_query($s);
    $s="update user_reg set repass='$pass' where us_id='$id'";  
    $re=mysql_query($s);
  ...
  else
  {
    ...
    $phno=$_POST['phno'];
      ...
      $re=mysql_query($s);
      $s="update emp_reg set pass='$pass' where emp_id='$id'";  
      $re=mysql_query($s);
      $s="update emp_reg set repass='$pass' where emp_id='$id'";  
      $re=mysql_query($s);
...
{
  $id=$_GET["mid"];
  $stat1=mysql_query("update pump_man_reg set status='0' where man_id='$id'");
  $stat2=mysql_query("update login_pump set status='0' where id='$id'");
  $stat1=mysql_query("update pump_man_reg set lic_status='-1' where man_id='$id'");
  $stat1=mysql_query("delete from pump_license where man_id='$id'");
...
if(isset($_GET["ammid"]))
{
  ...
  echo "<script>alert(' Manager Approved... ');</script>";
  echo "<script>location.href='admin_home.php';</script>";
}
if(isset($_GET["dmmid"]))
{
      ...
      if ($fileSize<500000) 
      {
        ...
        if($n==0)
        {
          ...
          if($m==0) 
          {
            ...
            mysql_query($sq);
            $s="update pump_man_reg set lic_status='1' where man_id='$id'";
            mysql_query($s);
...
function notapprovedemp()
{
  //session_start();
  $id=$_SESSION['id'];
  $p=mysql_query("select * from emp_appoint where man_id='$id'");
  $times=0;
  while($f=mysql_fetch_array($p)) 
      {
        $na=$f['emp_id'];
...
//--------------------2 headings excluding order_id from feedback_db table--------------------------------------
  ...
  {
    $o=$b['pump_id'];
    $q=mysql_query("select order_id from ordered_fuel where pump_id='$o'");
...
//--------------------row order_id inseting from feedback_db table--------------------------------------
	...
	Payed from ordered_fuel a, user_reg b, fuel_db c where b.us_id=a.us_id && c.fuel_id=a.fuel_id && a.pump_id = '655' order by a.date";
  ...
  { 
    ...
    { 
      ...
      if($n==1)
      {
          ...
          $sql="insert into pump_machine values('$mcid','$pid','0')";
          mysql_query($sql);
          echo "<script>alert(' Successfully Added ');</script>"; 
          echo "<script>location.href='manager_home.php';</script>";
...
if (isset($_POST['fuel_submit'])) 
{
  ...
  if($p==1)
  {
    ...
    if($m==1)
    {
      $sq="select ltr from fuel_at_pump where pump_id='$pid' && fuel_id='$fid'";
      $res=mysql_query($sq);   
      ...
      if($n==1)
      {
        ...
        $fltr=$fltr+$amount;
        $sql="update fuel_at_pump set ltr='$fltr' where pump_id='$pid' && fuel_id='$fid'";
        mysql_query($sql);
        echo "<script>alert(' Successfully $amount , $fltr ');</script>";
        echo "<script>location.href='manager_home.php';</script>";
    ...
    {
      echo "<script>alert(' Fuel cannot be added without approving your License by Admin ');</script>";
      echo "<script>location.href='manager_home.php';</script>";
    }
  }
  else
...
function showpump()
{
  $query = "select pump_id as Pump_Id,franchise_name as Franchise_Name,email as Email_Id,street as Street,district as District,state as State,pincode as Pincode,phno as Phone_No  from add_pump_db";
  $result = mysql_query($query);
  if (!$result)
  {
    $message = 'ERROR:' . mysql_error();
...
if(isset($_GET["pid"]))
{
  ...
  $id=$_SESSION['id'];
  $status=0;
  $stat=mysql_query("select * from emp_appoint where emp_id='$id'");
  ...
  if ($status==0) 
  {
    $stat1=mysql_query("select * from add_pump_db where pump_id='$pid'");
    $row=mysql_fetch_array($stat1);
       $stat2=mysql_query("update emp_appoint set pump_id=$pid where emp_id='$id'" );
    $stat3=mysql_query("update emp_appoint set req_send='1' where emp_id='$id'" );
  // $stat1=mysql_query("update login_pump set status='1' where id='$id'");
...
//-------------------- heading from ordered_fuel table--------------------------------------
    ...
    $s="select a.order_id as Order_ID,b.usname as user_name ,a.pump_id as Pump_ID,a.emp_id as Employee_ID,c.name as Fuel,a.date,a.adv_amt as Advance_Amount,a.total,a.qty as Quantity,a.status_d as Delivery,a.status_p as Payed from ordered_fuel a, user_reg b, fuel_db c where b.us_id=a.us_id && c.fuel_id=a.fuel_id";
    $r=mysql_query($s);
    $m=0;
    echo '<html><body> <table style="width:100%" class="table"><tr>';
    while ($m < mysql_num_fields($r))
...
//-------------------- heading from ordered_fuel table--------------------------------------
    $s="select pump_id,franchise_name,street,district,state,pincode,phno as phone_no from add_pump_db where man_id='$mid'";
  ...
  if (in_array($fileActualExt, $allowed)) 
  {
    if ($fileError===0) 
    {
      if ($fileSize<500000) 
      {
        ...
        {
          $sq="insert into emp_appoint(emp_id,qual_pic) values('$id','$fileNameNew')";
          mysql_query($sq);
...
if (isset($_POST['feedback_submit'])) 
{
    ...
    $oid=$_POST['orderid'];
    $reply=$_POST['message'];
    $sql=mysql_query("select * from ordered_fuel where us_id='$id'");
    $n=mysql_num_rows($sql);
    if($n>=1)
    { 
      $sql=mysql_query("select * from ordered_fuel where us_id='$id' && order_id='$oid'");
      $m=mysql_num_rows($sql);
      if($m==1)
      {
        $sql=mysql_query("select * from ordered_fuel where us_id='$id' && order_id='$oid' && emp_id=''");
        $q=mysql_num_rows($sql);
        if($q==0)
        {
          $sql=mysql_query("select * from feedback_db where order_id='$oid'");
          $p=mysql_num_rows($sql);
          if($p==0)
          {
            $sql="insert into feedback_db(order_id,descrip) values('$oid','$reply')";
            $result=mysql_query($sql);
          ...
          {
            echo "<script>alert('!!! Already Feedback is added with this Order ID !!! ');</script>";
            echo "<script>location.href='user_home.php';</script>";
          }
        }
        else
        {
          echo "<script>alert('!!! Feedback can only be added after Accepting the Order by the PUMP !!! ');</script>";
          echo "<script>location.href='user_home.php';</script>";
        }
      }
      else
      {
        echo "<script>alert('!!! You have not done an Order with this Order ID !!! ');</script>";
        echo "<script>location.href='user_home.php';</script>";
      }
    }
    else
...
function feedbackuser()
{
  ...
  //echo "$o";
    ...
    $s="select a.order_id,c.emp_name as Employee_name,e.man_name as Manager_Name,a.descrip as Description,a.reply from feedback_db a, ordered_fuel b, emp_reg c, add_pump_db d, pump_man_reg e";
    $r=mysql_query($s);
  ...
  {
    $o=$b['order_id'];
    $s="select a.order_id,c.emp_name as Employee_name,e.man_name as Manager_Name,a.descrip as Description,a.reply from feedback_db a, ordered_fuel b, emp_reg c, add_pump_db d, pump_man_reg e where a.order_id=$o && b.order_id=$o && b.emp_id=c.emp_id && b.pump_id=d.pump_id && e.man_id=d.man_id";
...
function pendingorders_user()
{
  ...
  else
  {
    ...
    {
//-------------------- heading from ordered_fuel table--------------------------------------
      $s="select a.order_id as Order_ID,a.pump_id,b.emp_name,c.name as Fuel ,a.date ,a.pincode as Delivery_Point,a.qty as litre, a.adv_amt as Advance_Amount ,a.total ,a.status_p as Payment,a.status_d as Delivery from ordered_fuel a, emp_reg b,fuel_db c where b.emp_id=a.emp_id && c.fuel_id=a.fuel_id && a.us_id=$id && a.status_d='0' && a.status_p!=0";
      $r=mysql_query($s);
      $br=mysql_num_rows($r);
...
//--------------------row order_id inseting from feedback_db table--------------------------------------
    ...
    {
      $s="select a.order_id as Order_ID,a.pump_id,c.name as Fuel ,a.date ,a.pincode as Delivery_Point,a.qty as litre, a.adv_amt as Advance_Amount ,a.total ,a.status_p as Payment,a.status_d as Delivery from ordered_fuel a, emp_reg b,fuel_db c where c.fuel_id=a.fuel_id && a.us_id=$id && a.status_d='0' && a.status_p!=0";
      $r=mysql_query($s);
      $br=mysql_num_rows($r);
...
function deliveredorders_user()
{
  ...
  {
    //-------------------- heading from ordered_fuel table--------------------------------------
      $s="select a.order_id as Order_ID,a.pump_id,b.emp_name,c.name as Fuel ,a.date ,a.pincode as Delivery_Point,a.qty as litre, a.adv_amt as Advance_Amount ,a.total ,a.status_p as Payment,a.status_d as Delivery from ordered_fuel a, emp_reg b,fuel_db c where b.emp_id=a.emp_id && c.fuel_id=a.fuel_id && a.emp_id=$id && a.status_d='1' && a.status_p=2";
      $r=mysql_query($s);
      $br=mysql_num_rows($r);
